...
func main() {
	...
	for i, c := range args_s {
		if Contains(vowels, c) {
			...
			vowel_pos_rev = append(vowel_pos_rev, i)
		}
	}
	if len(vowel_pos) == 0 {
	...
	for i := 0; i < len(vowel_pos)/2; i++ {
		args_s[vowel_pos[i]], args_s[vowel_pos_rev[len(vowel_pos_rev)-1-i]] = args_s[vowel_pos_rev[len(vowel_pos_rev)-1-i]], args_s[vowel_pos[i]]
	}
	for _, c := range args_s {
		z01.PrintRune(c)
	}
